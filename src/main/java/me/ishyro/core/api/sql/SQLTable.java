...
public class SQLTable {
	...
	public synchronized ResultSet select(String idValue) {
		return select(iDField, idValue);
	}

	public synchronized ResultSet select(String column, String columnValue) {
		final String sql = "SELECT * FROM " + getTableName() + " WHERE " + column + " = '" + columnValue + "'";
		return SQLHandler.sqlQuery(sql);
	}

	public synchronized void update(SQLUpdateColumn sqlUpdateColumn) {
	...
	public synchronized void increaseValue(final String idValue, final String fieldName, final int amount) {
		SQLHandler.sqlUpdate("UPDATE " + getTableName() + " SET " + fieldName + " = " + fieldName + " + " + amount + " WHERE " + iDField + " = '" + idValue + "'");
	}

	public synchronized void decreaseValue(final String idValue, final String fieldName, final int amount) {
		SQLHandler.sqlUpdate("UPDATE " + getTableName() + " SET " + fieldName + " = " + fieldName + " - " + amount + " WHERE " + iDField + " = '" + idValue + "'");
	}

	public synchronized boolean insertOrUpdate(SQLUpdateColumn sqlUpdateColumn) {
	...
	public synchronized boolean exist(final Object whereValue) {
